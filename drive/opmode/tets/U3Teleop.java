            ...
            if (isAutoControlled) {
                ...
                switch (machine) {
                    ...
                    case INTAKE_UP:
                        ...
                        if(passthrough.state != NewPassthrough.State.RETRACTED) {
                            passthrough.setTarget(NewPassthrough.State.RETRACTED);
                        }
                        drive_speed_constant = 1;//1

                        override_lift_update = false;

                        if(!override_lift_update /*&& passthrough.state == NewPassthrough.State.RETRACTED*/) {//uncomment crap???
                ...
                telemetry.addData("inspiring: ", passthrough.in_spring);
                telemetry.addData("done srping: ", passthrough.done_spring);
                telemetry.update();
